...
export class PlayerPlongeonGame {
    ...
    if (!this.isEndGame) {
      if (!this.gameActive) {
        if (!this.isTextPlongeonShow) {
          ...
          document
            .getElementById("plongeonGame-text-plongeon")!
  ...
  private processFigureInput(figure: {
    key: string;
    pose: () => void;
    letter: string;
  }): void {
  ...
  private setAnimation(): {
    ...
    const plongeon = this.animationsGroup.find(
      (ag) => ag.name === "Anim|jumpForward",
    );
  ...
  public runPoolAnim() {
    ...
    this.poolAnim.onAnimationEndObservable.addOnce(() => {
      ...
      this.transform.position = new Vector3(-1.172302484512329, 0.5, 16.5);
      // this.isEndGame = true;
    });
  ...
  public _updateGroundDetection(): void {
    ...
    if (!isGrounded) {
      this.transform.moveWithCollisions(
        new Vector3(0, PlayerPlongeonGame.GRAVITY, 0),
      );
    }
  }
