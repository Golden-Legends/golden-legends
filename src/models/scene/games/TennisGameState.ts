...
export class TennisGameState extends GameState {
  ...
  constructor(game: Game, canvas: HTMLCanvasElement, multi?: boolean) {
    super(game, canvas);
    ...
    try {
      ...
      if (cameraMesh) {
        this._camera.position = new Vector3(
          -cameraMesh.position.clone().x,
          cameraMesh.position.clone().y + 10,
          cameraMesh.position.clone().z,
        );
        this._camera.rotation = new Vector3(1.5, 1.563, 0);
...
class Ball {
  ...
  constructor(scene: Scene, paddle1, paddle2: Paddle, maxGame: number) {
    ...
    if (ballInitMeshPosition) {
      this.resetPlacement = new Vector3(
        ballInitMeshPosition.getAbsolutePosition().x,
        -0.2,
        ballInitMeshPosition.getAbsolutePosition().z,
      );
    }
  ...
  _move() {
    ...
    this._velocity.x = Math.min(
      this.maxSpeed,
      Math.max(-this.maxSpeed, this._velocity.x),
    );
    this._velocity.z = Math.min(
      this.maxSpeed,
      Math.max(-this.maxSpeed, this._velocity.z),
    );
  ...
  private _checkCollision() {
    ...
    if (
      Math.abs(ballPos.x - paddle1Pos.x) < WIDTH / 2 &&
    ...
    if (
      Math.abs(ballPos.x - paddle2Pos.x) < WIDTH / 2 &&
...
class Paddle {
  ...
  constructor(paddleType: string, scene: Scene, input: PlayerInputTennisGame) {
    ...
    mat.emissiveColor =
      this._type == "player" ? new Color3(0, 0, 1) : new Color3(1, 0, 0);
    ...
    if (player1Init && player2Init) {
      this._body.position =
        this._type == "player"
          ? player1Init.getAbsolutePosition().clone()
          : player2Init.getAbsolutePosition().clone();
    }
