...
import JavelotGameSettings from "@/../public/javelotGameSettings.json";
import {
  Animation,
  FreeCamera,
  HemisphericLight,
...
export class JavelotGameState extends GameState {
  ...
  private async initPlayer() {
    ...
    this.player = new PlayerJavelotGame(
  ...
  async CreateCameraMouv(): Promise<void> {
    ...
    rotationKeys.push({
      frame: 7 * fps,
      value: new Vector3(Math.PI / 10, (-5 * Math.PI) / 6, 0),
    });
  ...
  async showScoreBoard(): Promise<void> {
    ...
    setTimeout(() => {
      document
        ...
        .classList.add("hidden");
      document
        .getElementById("javelotGame-results")!
        .classList.remove("hidden");
    }, 2000);
  }
  ...
  createFinaleScoreBoard(): void {
    ...
    if (this.score >= this.settings.level[this.difficulty].pointToSucceed) {
      if (
        ...
        localStorage.getItem("levelJavelot") === "easy"
      ) {
        localStorage.setItem("levelJavelot", "intermediate");
      }
      if (
        this.difficulty === "intermediate" &&
        localStorage.getItem("levelJavelot") === "intermediate"
      ) {
