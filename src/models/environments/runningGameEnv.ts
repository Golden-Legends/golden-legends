...
export class runningGameEnv {
  ...
  public loadPublic() {
    for (let i = 0; i <= 11; i++) {
      ...
      if (i !== 0) {
        baseNames = baseNames.map((name, _) => {
          return i === 10 || i === 11 ? `${name}.0${i}` : `${name}.00${i}`;
        });
        baseNames.push(
          `persoO${randomNumbers[0]}.${i === 10 || i === 11 ? "0" : "00"}${i}`,
        );
      } else {
      ...
      for (let j = 0; j < baseNames.length; j++) {
        ...
        if (mesh) {
          ...
          const child = InstanceManager.duplicateParentMesh(
            this.parentMesh,
            `public${i}${randomNumbers[j % 3]}`,
          );
          const res = InstanceManager.duplicateInstance(
            this.assetContainerTab[randomNumbers[j % 3]],
            pos,
            new Vector3(0, offsets[j % offsets.length], 0),
            child,
            0.09,
          );
          const idle = res.animationGroups.find((ag) =>
  ...
  public loadPublicLateral() {
    for (let i = 0; i <= 1; i++) {
      ...
      if (i === 0) {
        ...
        let pos1 = this._scene.getMeshByName(
          "persoD" + randomNumber1 + ".00" + i,
        );
        let pos2 = this._scene.getMeshByName(
          "persoD" + randomNumber2 + ".00" + i,
        );
        let pos3 = this._scene.getMeshByName(
          "persoD" + randomNumber3 + ".00" + i,
        );
        if (pos1) {
          let position = new Vector3(
            -pos1.position.x,
        ...
        let posd1 = this._scene.getMeshByName(
          "persoF" + randomNumber1 + ".00" + i,
        );
        let posd2 = this._scene.getMeshByName(
          "persoF" + randomNumber2 + ".00" + i,
        );
        let posd3 = this._scene.getMeshByName(
          "persoF" + randomNumber3 + ".00" + i,
        );
        if (posd1) {
          let position = new Vector3(
            -posd1.position.x,
