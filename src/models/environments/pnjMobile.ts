...
export class PnjMobile {
	...
	public async init() {
        ...
        for (let i = 0; i < this.pnjPositions.length; i++) {
            ...
            await this._loadCharacterAssets(this._scene, position, fileName, "pnjMobile" + i, rotation);
        }   
    ...
    public async moveCharacter(num: number, direction: Vector3) {
        ...
        const walk = this.assets[num].animationGroups.find(ag => ag.name === "walk");
        if(walk){
        ...
        const idle = this.assets[num].animationGroups.find(ag => ag.name === "idle");
        if(idle){
    ...
    private async moveLinear(distance: Vector3, frames: number, anim: AnimationGroup, num: number) {
        const characterMesh = this._scene.getMeshByName("pnjMobile" + num) as Mesh; // Supposons que le personnage principal est nommé "outer"
        const fps = 30;
        ...
        const animation = new Animation("moveAnimation", "position", fps, Animation.ANIMATIONTYPE_VECTOR3, Animation.ANIMATIONLOOPMODE_CONSTANT);
